...
class HoneypotConfigStore(BaseStore):
    ...
    def get_config(self, guild_id: int) -> HoneypotConfig | None:
        ...
        row = self.db.execute(
            """
            SELECT guild_id, channel_id, action, action_role_id, alerts_enabled
    ...
    def set_action(
        self, guild_id: int, action: str, role_id: int | None = None
    ) -> None:
        """Persists the moderation action and optional target role."""
        self.db.execute(
            """
            INSERT INTO honeypot_config (guild_id, action, action_role_id)
            VALUES (?, ?, ?)
            ON CONFLICT(guild_id) DO UPDATE SET
