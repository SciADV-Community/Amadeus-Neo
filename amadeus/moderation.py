...
async def execute_action(
    ...
    if action == "mute":
        return await action_mute(member)
    if action == "kick":
        return await action_kick(member)
    if action == "ban":
        return await action_ban(member)
...
async def action_remove_role(
    ...
    try:
        await member.remove_roles(role, reason="Honeypot triggered")
        log(
...
async def action_mute(member: discord.Member) -> str:
    until = discord.utils.utcnow() + timedelta(days=28)

    try:
        await member.timeout(until, reason="Honeypot triggered")
        log(
...
async def action_kick(member: discord.Member) -> str:
    try:
        await member.kick(reason="Honeypot triggered")
        log(
...
async def action_ban(member: discord.Member) -> str:
    try:
        await member.ban(reason="Honeypot triggered", delete_message_days=0)
        log(
